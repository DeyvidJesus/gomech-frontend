...
export function AddVehicleModal({ onClose }: AddVehicleModalProps) {
  ...
  const mutation = useMutation({
    ...
    onError: () => setError("Erro ao cadastrar veículo."),
  });
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-onyx-200 p-8 rounded-lg shadow min-w-[320px]">
        ...
        <label className="block mb-2">ID do Cliente:
          <input name="clientId" value={form.clientId || ''} onChange={handleChange} required className="input input-bordered w-full mt-1" type="number" />
        </label>
        {error && <div className="text-red-600 mb-2">{error}</div>}
        <div className="flex gap-2 mt-4">
          <button type="submit" disabled={loading} className="bg-orangeWheel-500 hover:bg-orangeWheel-600 text-white px-4 py-2 rounded transition-colors">
