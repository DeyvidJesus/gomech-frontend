...
export function VehicleImportModal({ isOpen, onClose, onUpload }: VehicleImportModalProps) {
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!extensions.some(ext => selected.name.toLowerCase().endsWith(ext))) {
      setError("Formato inválido. Utilize CSV ou XLSX");
      setFile(null);
      return;
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setError(uploadError?.response?.data?.message || "Não foi possível importar os veículos");
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-start justify-between">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Importar veículos</h2>
            <p className="text-sm text-gray-500">Os veículos serão vinculados automaticamente aos clientes pelo ID informado.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
